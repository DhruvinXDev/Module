...
const updateUserValidation = [
  body('full_name').optional().trim().isLength({ min: 2, max: 50 }).withMessage('Full name must be between 2 and 50 characters'),
...
router.get('/', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const [users] = await pool.execute(
      'SELECT id, full_name, email, role, is_verified, avatar, created_at FROM users LIMIT ? OFFSET ?',
      [limit, offset]
    );
...
router.put('/:id', authenticateToken, updateUserValidation, async (req, res) => {
  try {
    ...
    const [updatedUsers] = await pool.execute(
      'SELECT id, full_name, email, role, is_verified, avatar, created_at, updated_at FROM users WHERE id = ?',
      [userId]
    );
